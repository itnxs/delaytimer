...
// Observable 转为 RxGo 可观察序列。关闭后先排空已进入缓冲的事件，再结束。
...
func (ec *EventChannel) forward(out chan<- rxgo.Item, item rxgo.Item) bool {
    select {
    case <-ec.ctx.Done():
        out <- item
        return false
...
func (ec *EventChannel) drain(out chan<- rxgo.Item) {
    for {
        select {
        case item, ok := <-ec.item:
            if !ok {
...
func observe(t *Timer, events rxgo.Observable, handle func(any), name string) {
    t.busDone.Add(1)
    disposed := events.ForEach(
        func(i interface{}) {
            handle(i)
        },
        func(err error) {
