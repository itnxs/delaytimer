...
func (h memHeap) Less(i, j int) bool { return h[i].job.At.Before(h[j].job.At) }
...
// Schedule 设置或覆盖任务
func (m *Memory) Schedule(_ context.Context, job Job) error {
	job = job.withKey()
	m.mu.Lock()
	defer m.mu.Unlock()

	if exist, ok := m.byKey[job.Key]; ok {
		exist.job = job
...
func (m *Memory) Claim(ctx context.Context, n int) ([]Job, error) {
	if n < 1 {
		n = 1
	}
	...
	for {
		...
		for len(out) < n && m.items.Len() > 0 {
			head := m.items[0]
			if head.job.At.After(now) {
				break
			}
			item := heap.Pop(&m.items).(*memItem)
			delete(m.byKey, item.job.Key)
...
func (m *Memory) Fail(_ context.Context, job Job) error {
	job = job.withKey()
	m.mu.Lock()
	defer m.mu.Unlock()
	if _, exists := m.byKey[job.Key]; exists {
		return nil
	}
	job.At = m.clock().Add(failRequeueDelay)
	m.pushLocked(job)
	return nil
}

func (m *Memory) pushLocked(job Job) {
	item := &memItem{job: job}
	heap.Push(&m.items, item)
	m.byKey[job.Key] = item
	m.cond.Broadcast()
}
